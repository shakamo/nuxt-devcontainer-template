...
const itemClick = (event: UIEvent, item: MenuItemType) => {
    if (item.disabled) {
...
const checkActiveRoute = (item: MenuItemType) => {
...
<template>
    <li :class="{ 'layout-root-menuitem': root, 'active-menuitem': isActiveMenu }">
        ...
        <nuxt-link v-if="item.to && !item.items && item.visible !== false" @click="itemClick($event, item)"
        ...
        <Transition v-if="item.items && item.visible !== false" name="layout-submenu">
            <ul v-show="root ? true : isActiveMenu" class="layout-submenu">
                <app-menu-item v-for="(child, i) in item.items" :key="child.label" :index="i" :item="child"
